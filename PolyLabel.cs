...
namespace FaaMvaToSectorFile
{
    ...
    {
        public MyPoint Centroid { get; set; }
        ...
        private static Cell GetCentroidCell(List<MyPoint> polygon)
        {
            float area = 0;
            float x = 0;
            float y = 0;
            List<MyPoint> points = polygon;
        ...
        class Cell
        {
            ...
            public Cell(float x, float y, float h, List<MyPoint> polygon)
            {
                this.X = x;
                this.Y = y;
                this.H = h;
            ...
            float PointToPolygonDist(float x, float y, List<MyPoint> polygon)
            {
                bool inside = false;
            ...
            float GetSeqDistSq(float px, float py, MyPoint a, MyPoint b)
            {
                float x = a.X;
                float y = a.Y;
                float dx = b.X - x;
